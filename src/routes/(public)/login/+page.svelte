    ...
    <div class="bg-sky-100 sm:w-96 mx-auto sm:mt-4 md:mt-20 mb-52 rounded p-4 shadow">
        ...
        <form class="flex flex-col gap-2" on:submit|preventDefault="{login}" method="post">
            <div class="flex flex-col font-semibold gap-1">
                ...
                <input  bind:value="{email}" type="email" class="py-1 px-4 text-sm rounded" name="email" id="email">
            </div>
            <div class="flex flex-col font-semibold gap-1">
                <label for="password">password</label>
                <input  bind:value="{password}" type="password" class="py-1 px-4 text-sm rounded" name="password" id="password">
            </div>
            <input type="submit" class="bg-sky-900 py-1 rounded my-2 text-white font-semibold uppercase cursor-pointer" value="login">
        </form>
    </div>
</Container>
{#if loading}
